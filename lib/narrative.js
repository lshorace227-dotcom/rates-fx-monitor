...
export function buildNarrativePrompt(base, { news = false } = {}) {
  ...
  const newsLines = news ? [
    `第一步：用 WebSearch 检索「${base.label}」近 1-2 周的重大新闻、经济数据与央行/监管动向（可多次搜索，优先权威财经源），把关键事实纳入下面的叙述。`,
    `在 JSON 顶层加 "sources":[{"title":"标题","url":"链接"}]，列出你实际引用的来源（3-6 条）。`,
  ] : [];
  const sourcesKey = news ? `"sources":[{"title":"...","url":"..."}],` : "";
  return [
    ...
    `{${sourcesKey}"recent_move_summary":"一句近况","key_risks":["..."],"watch_items":["..."],`,
    ...
    `每个情景 drivers 2-3 条、implication 一句，聚焦银行对公信贷视角，结合给定量化信号${news ? "与检索到的新闻" : ""}给出有依据的逻辑。`,
    `${news ? "完成检索后，最后只输出 JSON（不要输出搜索过程文字）。" : "只输出 JSON。"}`,
  ].join("\n");
}
