...
LPR、Shibor、HIBOR 3M/12M 没有稳定的免费实时源，需手动维护。用一键脚本，免手改 JSON：

```bash
npm run update        # 或 node update-data.mjs；macOS 也可双击 update.command
```

脚本会逐项显示**官方来源链接 + 当前值**，你对照官方填新值（直接回车=跳过该项），它自动按**当天日期**写入、同日重复则替换、保持升序。更新完重启 `node server.js` 生效。

- LPR：每月 20 日公布（www.chinamoney.com.cn）
- Shibor：每交易日 11:00（www.shibor.org）
